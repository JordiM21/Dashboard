...
export function StudentAvatar({ student, size = 44 }: { student: Student; size?: number }) {
...
export default function StudentsRoster() {
  ...
  return (
    <ErrorBoundary label="the students list">
      ...
      {data && filtered.length === 0 && <EmptyState title="No students match" hint="Try clearing filters, or add a new student." />}
      ...
      {editingStudent && <AddStudentModal editing={editingStudent} onClose={() => setEditingStudent(null)} onCreated={() => {}} />}
      {deleteTarget && (
        <ConfirmModal
